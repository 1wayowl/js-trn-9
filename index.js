...
function findIndexByProperty(arr, name) {
  ...
  return arr.findIndex((el) => el.name === name);
...
function sumAndPrint(arr) {
  ...
  let result = arr.reduce((num, elem) => num + elem, 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let filteredSquaredArray = arr2.filter((el) => el > threshold);
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) && typeof Number) {
  ...
  let expandedArray = arr.flatMap((n) => {
    return n;
  });
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((acc, el) => {
...
function findDuplicateElements(arr) {
  ...
  let duplicateElements = arr.filter((el, index) => index !== arr.indexOf(el));
...
function capitalizeWords(arr) {
  ...
  let capitalizedArray = arr.map((el) => {
    let F = el[0].toUpperCase();
    let Other = el.slice(1);
    return F + Other;
  });
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce((sum, elem) => sum + elem.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  ...
  let lastIndex = arr.findLastIndex((ind) => ind < 0);
